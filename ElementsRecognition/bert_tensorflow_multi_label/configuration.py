...
# do_eval = False
do_predict = True
do_raw_text_process = False


#
stop_word_path = "./"  # 停止词的位置
stop_word_filename = "stopwords_modify.txt"  # 停止词文件名stopwords_modify.txt

#  数据存放位置信息
task_type_name = "loan"  # 需要对3种数据进行训练： loan ， divorce ， labor
num_labels = 20

# 原始数据
raw_data_base_dir = "/home/liujiepeng/MachineComprehension/CAIL2019/ElementsRecognition/data/CAIL2019-FE-big/" + task_type_name
raw_data_filename = "train_selected.json"  # 对于labor数据集建议采用修正过的数据，因为原始的数据有些垃圾信息。
tag_file = os.path.join(raw_data_base_dir, "tags.txt")

# 基于原始数据生成的中间数据
data_base_dir = os.path.join("./data", task_type_name)
